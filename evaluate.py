...
class Evaluator(object):
    def __init__(self, cfg):
        ...
        self.model.load_dict(pretrained_net)
    ...
    def eval(self):
        ...
        for i in range(len(self.image_paths)):
            ...
            image = paddle.unsqueeze(image, 0)                      # image shape: (1,3,H,W) [0,1]
            ...
            lsit_pixAcc.append(pixAcc.item())
            list_mIoU.append(mIoU.item())
            logger.info("Sample: {}, validation pixAcc: {}, mIoU: {}, time: {}s".format(
            ...
            # save pred 
            pred = paddle.argmax(outputs[0], 1)
            pred = pred.numpy()
            pred = pred.squeeze(0)
            pred = get_color_pallete(pred, "citys")
            pred.save(os.path.join(outdir, prefix + "_mIoU_{}.png".format(mIoU)))
        ...
        logger.info("Evaluate: Average mIoU: {}, Average pixAcc: {}, Average time: {}".format(average_mIoU, average_pixAcc, average_time))
    ...
    def _mask_transform(self, mask):
        mask = self._class_to_index(np.array(mask).astype('int32'))
        return paddle.Tensor(np.array(mask).astype('int32'))
